...
const ADDRESS_ACTION = 'notification-emails';
...
> {
  ...
  // Admin-only, and not a section that can be granted: who gets told about an
  // enquiry is a different decision from who may read one.
  if (user.role !== 'ADMIN') return { ok: false, message: 'Only an Admin can change these.' };
  return { ok: true, user };
}
...
export async function addNotificationEmail(
  ...
  if (isRateLimited(`${ADDRESS_ACTION}:${auth.user.id}`, ADMIN_REQUESTS_PER_WINDOW)) {
  ...
  await recordAudit({
    user: auth.user,
    action: 'notification_email.added',
    entity: 'NotificationEmail',
    entityId: row.id,
    hotelSlug,
    summary: `Added a notification address to ${where}`,
    after: { hotelSlug, address },
    ip: clientIp(await headers()),
  });

  log.info('notification_email.added', { hotel: hotelSlug });
  revalidatePath('/admin/enquiries');
  return { status: 'success', message: `Added to ${where}.` };
}
...
export async function removeNotificationEmail(
  ...
  await recordAudit({
    user: auth.user,
    action: 'notification_email.removed',
    entity: 'NotificationEmail',
    entityId: row.id,
    hotelSlug: row.hotelSlug,
    summary: `Removed a notification address from ${where}`,
    before: { hotelSlug: row.hotelSlug, address: row.address },
    ip: clientIp(await headers()),
  });

  log.info('notification_email.removed', { hotel: row.hotelSlug });
  revalidatePath('/admin/enquiries');
  return { status: 'success', message: `Removed from ${where}.` };
}
