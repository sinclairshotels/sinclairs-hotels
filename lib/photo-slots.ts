...
// Home, then every hotel in site order, then the enquiry and contact pages, then
// the remaining marketing pages. "Not used on any page" is built by difference
// in lib/photo-files.ts, so it is only ever as correct as this list.
export function photoPages(): PhotoPage[] {
  return [HOME, ...hotels.map(hotelPage), ENQUIRY, CONTACT, ...OTHER_PAGES];
}
